...
show_menu() {
    ...
    echo -e "[${BLUE}9${NC}] ${WHITE}$fleet_version{NC}"
...
reset_all() {
    for product in $intellij_idea_version $webstorm_version $datagrip_version $phpstorm_version $clion_version $pycharm_version $goland_version $rubymine_version $fleet_version; do
         ...
         local application_name=""
            case $application in
                ...
                8)
                    ...
                    ;;
                9)
                    application_name=$fleet_version
                    ;;
            esac
